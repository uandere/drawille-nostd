//! `drawille-nostd` – a `no_std` version of the [drawille](https://github.com/ftxqxd/drawille-rs)
...
pub struct Canvas {
    chars: BTreeMap<(u16, u16), (u8, char, bool)>,
    width: u16,
    height: u16,
}
...
impl Canvas {
    ...
    pub fn set(&mut self, x: u32, y: u32) {
        ...
        let a = self.chars.entry((row, col)).or_insert((0, ' ', false));
    ...
    pub fn rows(&self) -> Vec<String> {
        ...
        for y in 0..=maxcol {
            ...
            for x in 0..=maxrow {
                let cell = self.chars.get(&(x, y)).cloned().unwrap_or((0, ' ', false));
                match cell {
                    (0, _, _) => row.push(cell.1),
                    (_, _, false) => row.push(char::from_u32(0x2800 + cell.0 as u32).unwrap()),
                    (_, _, true) => {
                        row = format!(
                            "{0}{1}",
                            row,
